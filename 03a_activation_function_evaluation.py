    ...
    for layer_size, num_passes in zip(layer_sizes, iterations):
        ...
        for size in layer_size:
            layers.append(FullyConnected(
                ...
                weight_initializer=RandomUniform(low=-np.sqrt(1.0/size), high=np.sqrt(1.0/size)),
                fb_weight_initializer=RandomUniform(low=-np.sqrt(1.0/size), high=np.sqrt(1.0/size))
            ))
        layers.append(FullyConnected(size=10, activation=None, last_layer=True))
